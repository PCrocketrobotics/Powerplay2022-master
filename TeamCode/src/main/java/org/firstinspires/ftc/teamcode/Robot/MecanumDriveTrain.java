...
public class MecanumDriveTrain {
    ...
    static final double         DRIVE_GEAR_REDUCTION   = 15.2;    // Rev Ultraplanetary Motor 12:1 but actual is 10.5:1
    ...
    static final double         P_DRIVE_COEFF          = 0.055; //Originally 0.15
    ...
    public void init() {
        ...
        left_front  = robot.opMode.hardwareMap.get(DcMotorEx.class, "leftFront");
        left_back   = robot.opMode.hardwareMap.get(DcMotorEx.class, "leftBack");
        right_front = robot.opMode.hardwareMap.get(DcMotorEx.class, "rightFront");
        right_back  = robot.opMode.hardwareMap.get(DcMotorEx.class, "rightBack");
       ...
       // Set Direction of Each Drive Motor to keep Speeds identical between all motors.
        ...
        right_back.setDirection(DcMotorEx.Direction.FORWARD);
    ...
    public void DriverControlled_Drive(double speedreduction){
        ...
        //double x = robot.opMode.gamepad1.left_trigger - robot.opMode.gamepad1.right_trigger;
        double rx = robot.opMode.gamepad1.right_stick_x;

        double denominator = Math.max(Math.abs(y) + Math.abs(x) + Math.abs(rx), 1);
        double power_left_front = (y + x + rx)/ denominator;
        double power_left_back = (y - x + rx)/ denominator;
        double power_right_front = (y - x - rx)/ denominator;
        double power_right_back = (y + x - rx)/ denominator;
        ...
        robot.opMode.telemetry.addLine("Front Wheel Power")
                .addData("Left",  "%7d", left_front.getCurrentPosition())
                .addData("Right", "%7d", right_front.getCurrentPosition());
        robot.opMode.telemetry.addLine("Rear Wheel Power")
                .addData("Left",  "%7d", left_back.getCurrentPosition())
                .addData("Right", "%7d", right_back.getCurrentPosition());
    ...
    public void imuTurnGlobal(double targetangle, boolean eBrake){
        ...
        while ((currentangle > maxerror || currentangle < minerror) && !eBrake){
            ...
            if (currentangle > targetangle){
                robot.mecanumDriveTrain.left_front.setPower(0.3   * throttle);
                robot.mecanumDriveTrain.left_back.setPower(0.3    * throttle);
                robot.mecanumDriveTrain.right_front.setPower(-0.3 * throttle);
                robot.mecanumDriveTrain.right_back.setPower(-0.3  * throttle);
            }
            if (currentangle < targetangle){
                robot.mecanumDriveTrain.left_front.setPower(-0.3 * throttle);
                robot.mecanumDriveTrain.left_back.setPower(-0.3  * throttle);
    ...
    public void gyroDrive(double left_front_distance, double left_back_distance,
                          ...
                          double motor_power, double angle) {
        ...
        // Ensure that the opmode is still active
            ...
            // start motion.
            motor_power = Range.clip(Math.abs(motor_power), 0.0, 1);
            left_front.setPower(motor_power);
            right_front.setPower(motor_power);
            left_back.setPower(motor_power);
            right_back.setPower(motor_power);
            ...
            while (left_front.isBusy() && right_front.isBusy() && left_back.isBusy() && right_back.isBusy()) {
                ...
                left_front.setPower(left_front_speed);
                right_front.setPower(right_front_speed);
                left_back.setPower(left_back_speed);
                right_back.setPower(right_back_speed);
    ...
    public double getSteer(double error, double PCoeff) {
        return Range.clip(error * PCoeff, -.75, .75);
    }
     ...
     * Get cardinal directions (vector) or starting orientation (vector)
     * Individually subtract cardinal directions from current orientation (int degrees)
     * Choose closes angle
     * Apply difference as turn towards closest cardinal directions
     }
